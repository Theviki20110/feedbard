...
def test_an_empty_answer_is_how_the_model_declines(monkeypatch):
    # The prompt tells it to answer with nothing rather than explain why it
    # will not translate: a sentence about the task reads exactly like article
    # prose downstream and would be narrated as if the author had written it.
    calls = []

    def always_declines(prompt):
        calls.append(prompt)
        return "   ", 0.0

    monkeypatch.setattr(translator, "generate_response", always_declines)

    assert translate_chunk("hello", "Italian", 0) == ""
    assert len(calls) == translator.MAX_EMPTY_RETRIES + 1


def test_a_declined_block_is_asked_again_before_being_lost(monkeypatch):
    # Declining is sampled, not deterministic, and this stage has no
    # mechanical fallback: one bad draw should not delete a paragraph.
    responses = iter([("", 0.0), ("ciao mondo", 0.1)])
