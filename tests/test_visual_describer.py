...
def test_save_and_load_roundtrip():
    save_visual_shard("aaaaaaaaaa", "essenziale", "una curva", language="Italian")

    shard = load_visual_shard("aaaaaaaaaa", language="Italian")
    assert shard == {"klass": "essenziale", "description": "una curva", "language": "Italian"}


def test_shard_in_another_language_is_a_miss():
    save_visual_shard("aaaaaaaaaa", "essenziale", "a curve", language="English")
...
def test_shard_without_a_language_field_is_taken_at_face_value(_sandboxed_shards):
    ...
    path.write_text(json.dumps({"klass": "essenziale", "description": "una curva"}))
...
def test_describe_visual_parses_a_bare_json_reply(monkeypatch):
    monkeypatch.setattr(
        visual_describer, "_fetch_image", lambda vid, url: (b"bytes", "image/png")
    )
    monkeypatch.setattr(
        visual_describer,
        "generate_vision_response",
        lambda prompt, image_bytes, media_type: (
            '{"klass": "essenziale", "description": "La curva sale."}',
            0.0,
        ),
    )
    v = _visual()
    describe_visual(v)
    assert v.klass == "essenziale"
...
def test_describe_visual_extracts_json_from_surrounding_prose(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "generate_vision_response",
        lambda prompt, image_bytes, media_type: (
            'Ecco il risultato: {"klass": "illustrativo", "description": "Un ritratto."} Fine.',
            0.0,
        ),
    )
    v = _visual()
    describe_visual(v)
    assert v.klass == "illustrativo"
...
def test_describe_visual_blank_description_becomes_none(monkeypatch):
    ...
    monkeypatch.setattr(
        visual_describer,
        "generate_vision_response",
        lambda prompt, image_bytes, media_type: ('{"klass": "decorativo", "description": ""}', 0.0),
    )
    v = _visual()
    describe_visual(v)
...
def test_describe_visual_renders_hint_alt_and_caption_into_the_prompt(monkeypatch):
    ...
    def generate_vision_response(prompt, image_bytes, media_type):
        seen["prompt"] = prompt
        return '{"klass": "essenziale", "description": "d"}', 0.0
...
def test_a_failing_visual_falls_back_to_decorativo_instead_of_aborting(monkeypatch):
    def boom(vid, url):
    ...
    assert doc.visuals["a"].klass == "decorativo"
...
def test_a_second_run_resumes_every_visual_from_shards(monkeypatch):
    ...
    assert len(calls) == 1
    assert doc2.visuals["a"].klass == "essenziale"
...
def test_fetch_image_downloads_when_not_cached(monkeypatch, tmp_path, content_type, expected):
    ...
    image_bytes, media_type = visual_describer._fetch_image("aaaaaaaaaa", "https://img.example/a.png")
